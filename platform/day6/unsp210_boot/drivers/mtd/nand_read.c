...
//add by xiuhai
static int nand_read_blocks_k(ulong dst_addr, ulong size)
{
        uchar *buf = (uchar *)dst_addr;
        ulong i;
	 ulong kernel_nand_start = 0x600000;
	int skipped_page = 0;
	uint page_shift = 11;

        /* Read pages */
		//i±íÊ¾Ò»Ò³
        for (i = (kernel_nand_start>>page_shift); i < (size>>page_shift)+(kernel_nand_start>>page_shift); i++, buf+=(1<<page_shift)) {
		if ((i & (64 - 1)) == 0) {
			while (nand_isbad(i + skipped_page)) {
				skipped_page += 64;
			}
...
int  copy_kernel_toram(void)
{
	...
	/* read NAND blocks */
	rv = nand_read_blocks_k(0x30008000, 0x500000);
	return 0;
}
