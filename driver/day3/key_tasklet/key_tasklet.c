...
static int major;		
static int minor;		
static struct class *key_tasklet_class;
static struct device *key_tasklet_dev;
...
/*3.编写微线程处理函数*/
void tasklet_func(unsigned pm2_5)
{
	printk("%d\n",pm2_5);
}

irqreturn_t key_tasklet_fun(int irq, void *dev_id)
{
	int id_data = 0;
	id_data = (int)dev_id;
	printk("%s %d\n",__func__, id_data);
...
static ssize_t key_tasklet_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	int cpy_size;
	int retval;
	/*键值返回给应用*/
...
static struct file_operations key_tasklet_fops = {
	owner:		THIS_MODULE,
	read:		key_tasklet_read,
};
...
static int __init key_eint_init(void)
{
	int retval = 0;
	/*传递过去的参数*/
	unsigned long tasklet_num = 50;
	
	io_port_init();
	/*2.初始化微线程*/
	tasklet_init(&key_tasklet,tasklet_func,tasklet_num);

	/*设置中断触发方式*/
	set_irq_type(IRQ_EINT(20), IRQ_TYPE_EDGE_FALLING);
	/*中断申请*/
	request_irq(IRQ_EINT(20), key_tasklet_fun, IRQF_DISABLED, "heihei", (void *)DEV_ID);
	
	/* 注册字符设备驱动，返回成功分配的主设备号， 失败返回错误值 */
	major = register_chrdev(major, DRIVER_NAME, &key_tasklet_fops);
	if(major < 0){
	...
	key_tasklet_class = class_create(THIS_MODULE,DRIVER_NAME);
	if(IS_ERR(key_tasklet_class)){
		err("class create failed!");
		retval =  PTR_ERR(key_tasklet_class);
		goto error_class;
	}
	
	/*创建设备文件，通知用户在“/dev/”目录下创件名字为DRIVER_NAME的设备文件*/
	key_tasklet_dev = device_create(key_tasklet_class,NULL, MKDEV(major, minor), NULL, DRIVER_NAME);
	if(IS_ERR(key_tasklet_dev)){
		err("device create failed!");
		retval = PTR_ERR(key_tasklet_dev);
		goto error_device;
	}
...
error_device:
	class_destroy(key_tasklet_class);
error_class:
...
static void __exit key_eint_exit(void)
{
	...
	/*5.销毁微线程*/
	unregister_chrdev(major, DRIVER_NAME);
	device_destroy(key_tasklet_class,MKDEV(major, minor));
	class_destroy(key_tasklet_class);
